...
<div class="container my-5">
    <h2 class="text-center mb-4">Productos de la categoría Audífonos</h2>

    <!-- Botón para abrir el modal de creación -->
    <button type="button" class="btn btn-primary mb-4" data-bs-toggle="modal" data-bs-target="#createAudifonoModal">
        Crear Nuevo Producto
    </button>

    <!-- Modal para crear un nuevo producto en la categoría Audífonos -->
    <div class="modal fade" id="createAudifonoModal" tabindex="-1" aria-labelledby="createAudifonoModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content bg-dark text-light">
                <div class="modal-header">
                    <h5 class="modal-title" id="createAudifonoModalLabel">Crear Nuevo Producto - Audífonos</h5>
                ...
                <div class="modal-body">
                    <form action="{{ route('products.store') }}" method="POST">
                        @csrf
                        <div class="mb-3">
                            <label for="nameCreate" class="form-label">Nombre del Producto</label>
                            <input type="text" class="form-control" id="nameCreate" name="name" required>
                        </div>
                        <div class="mb-3">
                            <label for="priceCreate" class="form-label">Precio</label>
                            <input type="number" class="form-control" id="priceCreate" name="price" required step="0.01" min="0">
                        </div>
                        <div class="mb-3">
                            <label for="stockCreate" class="form-label">Stock</label>
                            <input type="number" class="form-control" id="stockCreate" name="stock" required min="0">
                        </div>
                        <div class="mb-3">
                            <label for="imageCreate" class="form-label">URL de la Imagen</label>
                            <input type="text" class="form-control" id="imageCreate" name="image" required>
                        </div>
                        <input type="hidden" name="category" value="audifonos">
    ...
    <!-- Lista de productos existentes en la categoría Audífonos -->
    <div class="row">
                                    ...
                                    <div class="modal-content bg-dark text-light">
                                        ...
                                        <div class="modal-body">
                                            <form action="{{ route('products.update', $product->id) }}" method="POST">
                                                ...
                                                @method('PUT')
                                                <div class="mb-3">
                                                    <label for="nameEdit{{ $product->id }}" class="form-label">Nombre del Producto</label>
                                                    <input type="text" class="form-control" id="nameEdit{{ $product->id }}" name="name" value="{{ $product->name }}" required>
                                                </div>
                                                <div class="mb-3">
                                                    <label for="priceEdit{{ $product->id }}" class="form-label">Precio</label>
                                                    <input type="number" class="form-control" id="priceEdit{{ $product->id }}" name="price" value="{{ $product->price }}" required step="0.01" min="0">
                                                </div>
                                                <div class="mb-3">
                                                    <label for="stockEdit{{ $product->id }}" class="form-label">Stock</label>
                                                    <input type="number" class="form-control" id="stockEdit{{ $product->id }}" name="stock" value="{{ $product->stock }}" required min="0">
                                                </div>
                                                <div class="mb-3">
                                                    <label for="imageEdit{{ $product->id }}" class="form-label">URL de la Imagen</label>
                                                    <input type="text" class="form-control" id="imageEdit{{ $product->id }}" name="image" value="{{ $product->image }}" required>
                                                </div>
                                                <input type="hidden" name="category" value="audifonos">
                                                <div class="modal-footer">
                                                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
