...
<div class="container my-5">
    <h2 class="text-center mb-4">Productos de la categoría Escritorio</h2>

    <!-- Botón para abrir el modal de creación -->
    <button type="button" class="btn btn-primary mb-4" data-bs-toggle="modal" data-bs-target="#createEscritorioModal">
        Crear Nuevo Producto
    </button>

    <!-- Modal para crear un nuevo producto en la categoría Escritorio -->
    <div class="modal fade" id="createEscritorioModal" tabindex="-1" aria-labelledby="createEscritorioModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content" style="background-color: rgba(31, 41, 55, 1); color: rgba(209, 213, 219, 1);">
                <div class="modal-header">
                    <h5 class="modal-title" id="createEscritorioModalLabel">Crear Nuevo Producto - Escritorio</h5>
                ...
                <div class="modal-body">
                    <form action="{{ route('products.store') }}" method="POST">
                        ...
                        <input type="hidden" name="category" value="escritorio">
    ...
    <!-- Lista de productos existentes en la categoría Escritorio -->
    <div class="row">
                ...
                <div class="card d-flex flex-row align-items-center p-3" style="background-color: rgba(31, 41, 55, 1); color: rgba(209, 213, 219, 1); border-radius: 15px;">
                    ...
                    <div class="card-body">
                        ...
                        <div class="d-flex gap-2">
                            ...
                            <button type="button" class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#editProductModal-{{ $product->id }}">Editar</button>
                    ...
                    <div class="modal-content" style="background-color: rgba(31, 41, 55, 1); color: rgba(209, 213, 219, 1);">
                        ...
                        <div class="modal-body">
                            <form action="{{ route('products.update', $product) }}" method="POST">
                                ...
                                @method('PUT')
                                <div class="mb-3">
                                    <label for="name-{{ $product->id }}" class="form-label">Nombre del Producto</label>
                                    <input type="text" class="form-control" id="name-{{ $product->id }}" name="name" value="{{ $product->name }}" required>
                                </div>
                                <div class="mb-3">
                                    <label for="price-{{ $product->id }}" class="form-label">Precio</label>
                                    <input type="number" class="form-control" id="price-{{ $product->id }}" name="price" value="{{ $product->price }}" required step="0.01" min="0">
                                </div>
                                <div class="mb-3">
                                    <label for="stock-{{ $product->id }}" class="form-label">Stock</label>
                                    <input type="number" class="form-control" id="stock-{{ $product->id }}" name="stock" value="{{ $product->stock }}" required min="0">
                                </div>
                                <div class="mb-3">
                                    <label for="image-{{ $product->id }}" class="form-label">URL de la Imagen</label>
                                    <input type="text" class="form-control" id="image-{{ $product->id }}" name="image" value="{{ $product->image }}" required>
                                </div>
                                <input type="hidden" name="category" value="escritorio">
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
